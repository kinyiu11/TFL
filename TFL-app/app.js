...
const summaryPanelEl = document.querySelector(".summary");
...
const i18n = {
  ...
  en: {
    ...
    refresh: "Refresh",
    reselectStop: "Choose another stop",
...
const searchStops = async () => {
  ...
  try {
    ...
    const matches = Array.isArray(data.matches) ? sortStopsByName(data.matches).slice(0, 12) : [];
...
const searchLines = async () => {
  ...
  selectedLine = null;
  selectedStop = null;
  reselectStopButton.classList.add("hidden");
...
const fetchStopsForLine = async (lineId) => {
  ...
  try {
    ...
    const normalized = sortStopsByName(
      stops
      .map((item) => ({
...
const selectStop = (stop) => {
  ...
  stopListEl.innerHTML = "";
  if (summaryPanelEl) {
    summaryPanelEl.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  fetchArrivals();
};
...
const fetchStopSuggestions = async () => {
  ...
  try {
    ...
    const data = await res.json();
    const matches = Array.isArray(data.matches) ? sortStopsByName(data.matches).slice(0, 6) : [];
...
reselectStopButton.addEventListener("click", () => {
  selectedStop = null;
  selectedStopEl.textContent = t("noStopSelected");
  heroStopEl.textContent = t("noStopSelected");
  arrivalListEl.innerHTML = "";
